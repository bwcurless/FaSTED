...
\newcommand\whiteSpace{0.3}

	% Compute width of a single register based on the fraction of a single chunk it takes up.
\pgfmathsetmacro\registerWidth{0.375}
\pgfmathsetmacro\registerHeight{0.1875}
		...
		\foreach \row in {0,  1} {
			\foreach \col in {0,  1} {
				...
				\begin{scope}[xshift=\xoffset cm, yshift=\yoffset cm]
					...
					\node[anchor=south, font=\tiny] at (\registerWidth * 2, 0) {Phase \phaseNum};
...
\newcommand\bbPad{0.7}
\newcommand\drawLabeledBoundingBox[2]{%
	\draw[#1] ($(current bounding box.north west) + (-\bbPad,\bbPad)$) rectangle ($(current bounding box.south east) + (\bbPad, -\bbPad)$) node[anchor=south east, color=black, yshift=-0.125 cm] {#2};
}
