...
\begin{tikzpicture}
    % Draw global memory layouts
				...
				\tikzstyle nodeStyle=[anchor=south east, align=center, font=\tiny]
				\draw[xshift=-2.25 cm, yshift=0 cm, style=fragment, draw=red] (0, 0) rectangle +(\kHeight, -\mHeight) node[style=nodeStyle] {Query Points\\ (DRAM)};
				\draw[xshift=0 cm, yshift=0.25 cm, style=fragment, draw=blue] (0, \kHeight) rectangle +(\mHeight, -\kHeight) node[style=nodeStyle] {Candidate Points\\ (DRAM)};
				...
				% inner square rasterized chunks
										...
										\foreach \row in {0, ..., \innerRange} {
														\foreach \col in {0, ..., \innerRange} {
																		...
																		\draw[style=fragment, draw=black] (TileUL) rectangle ++(\tileDim, -\tileDim) node[midway, font=\tiny, align=center] {Block\\ Tile\\ \tileIndex};
...
% Warp Tile
\begin{tikzpicture}
				% Write a routine to draw a rectangle, and stack it a variable number of times.
